...
const iconMap = {
  FaHandshake, FaLightbulb, FaUserFriends, FaTools, FaDollarSign,
};
const iconList = [FaHandshake, FaLightbulb, FaUserFriends, FaTools, FaDollarSign];
...
const Objectives = () => {
  ...
  const { mission, vision, values, objectives } = content.objectives;
  ...
  return (
    <section id="about" ref={ref} className="py-24 bg-gray-950 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <motion.div className="text-center mb-20" initial="hidden" animate={isInView ? "visible" : "hidden"} variants={fadeUpObj}>
        ...
        {/* Mission & Vision */}
        <motion.div className="grid md:grid-cols-2 gap-8 mb-20" initial="hidden" animate={isInView ? "visible" : "hidden"} variants={staggerContainer}>
          <motion.div variants={fadeUpObj} className="bg-gray-900/50 border border-white/5 p-8 sm:p-10 rounded-2xl backdrop-blur-sm">
            ...
            <p className="text-gray-400 text-lg leading-relaxed">{mission}</p>
          </motion.div>
          <motion.div variants={fadeUpObj} className="bg-gray-900/50 border border-white/5 p-8 sm:p-10 rounded-2xl backdrop-blur-sm">
            <h3 className="text-2xl font-bold text-white mb-4 flex items-center gap-3">
            ...
            <p className="text-gray-400 text-lg leading-relaxed">{vision}</p>
          </motion.div>
        </motion.div>

        {/* Core Values */}
        <motion.div className="mb-20" initial="hidden" animate={isInView ? "visible" : "hidden"} variants={staggerContainer}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
            {values.map((v, i) => {
              ...
              return (
                <motion.div key={v.id} variants={fadeUpObj} className="bg-gray-900/30 border border-white/5 p-6 rounded-xl text-center group hover:bg-gray-800/50 transition-colors">
        ...
        <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} variants={staggerContainer}>
          ...
          <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
            {objectives.map((obj, i) => {
              const Icon = iconList[i % iconList.length];
              return (
                <motion.div key={obj.id} variants={fadeUpObj} className="flex gap-4 p-6 sm:p-8 bg-gray-900/30 border border-white/5 rounded-xl hover:border-indigo-500/30 transition-colors">
